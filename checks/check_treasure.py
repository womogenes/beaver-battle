...
for index, board in enumerate(BOARDS):
    ...
    for runner in (left, right):
        ...
        assert blocked or wet > .2, f'{board["name"]}: the straight line is free, so there is nothing to solve'
...
run(game, 25, lambda: tracing(game))
...
assert left.travelled - before > walked * 1.5 and "boost" in game.sounds and left.cooldown > 0
...
for index in range(len(BOARDS)):
    ...
    times[index] += elapsed
assert 20 < sum(times) / len(times) < 45, times
...
print(f"Treasure checks passed: {len(BOARDS)} fair mirrored boards, gap merging, broken paths, rocks, tracing, water, boost, "
