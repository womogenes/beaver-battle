...
from beaver_battle.bots import DamAttacker, DamBot
...
def attack(game, limit=90.0, human=False):
    """Set the bot attacker on a dam: the perfect one, or (human=True) one that takes a moment to read the board and
    loses a frame in five to steering, as a careful person does. Returns the seconds the flood took, or None if the
    lodge stayed dry."""
    bot, elapsed = DamAttacker(2, 1.2 if human else 0.0, 5 if human else 0), 0.0
    while game.phase == "attack" and elapsed < limit:
        control = bot.step(game, 1 / 60)
        game.update(1 / 60, {} if control is None else {2: control})
        elapsed += 1 / 60
