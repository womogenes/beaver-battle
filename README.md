...
4. Run `uv run python -m beaver_battle --display 1 --fullscreen --calibrate` (use the verified projector and camera indices). Four projected ArUco markers establish the camera mapping. While it waits, the status line names the corners the camera is missing, so the mount can be aimed from the projection itself. Calibration saves locally and returns to the lobby with `Calibration saved.` Repeat whenever the camera/projector moves or keystone changes.
5. Draw on the board in black marker before or after calibrating. Calibration reads the board from the marker screen, so anything drawn is solid as soon as calibration finishes; closed outlines fill in as logs and rocks, and open strokes stay as solid sticks. Drawings continue to update during play at `camera.wall_update_hz`, and so do hands, shadows and objects held in the beam: anything that takes away light the projector sent becomes solid while it is there and clears when it is removed. Draw in **green or black**, which fill in as solid bodies. Blue works as a barrier line but will not reliably fill a closed outline. **Red, orange and pink cannot be detected at all** — they reflect red light just as the laser dots do, so they are excluded on purpose rather than left ambiguous. See [camera and calibration](docs/vision.md) for the measurements.
