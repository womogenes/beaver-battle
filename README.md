...
5. Draw on the board in black marker before or after calibrating. Calibration reads the board from the marker screen, so anything drawn is solid as soon as calibration finishes; closed outlines fill in as logs and rocks, and open strokes stay as solid sticks. Drawings continue to update during play at `camera.wall_update_hz`, and so do hands, shadows and objects held in the beam: anything that takes away light the projector sent becomes solid while it is there and clears when it is removed. Keep drawings in black or another neutral dark color: saturated red is reserved for laser dots.
