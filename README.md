...
5. Draw on the board in black marker before or after calibrating. Calibration reads the board from the marker screen, so anything drawn is solid as soon as calibration finishes; closed outlines fill in as logs and rocks, and open strokes stay as solid sticks. Drawings continue to update during play at `camera.wall_update_hz`. Keep drawings in black or another neutral dark color: saturated red is reserved for laser dots.
6. Choose a two- or three-player match; each player presses button 2 to ready. After the countdown, steer by pointing at the board. The game periodically isolates one laser to identify it; brief blinking is expected.
