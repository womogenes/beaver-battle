...
BOARDS = [
    ...
    {"name": "ROCK GARDEN", "rocks": [(.20, .30, .03), (.20, .53, .03), (.20, .76, .03), (.29, .41, .03), (.29, .65, .03),
     ...
     "ponds": [(.12, .22, .06, .10), (.12, .84, .06, .10), (.29, .20, .05, .07), (.29, .86, .05, .07)]},
...
class Treasure:
    ...
    def new_match(self, player_ids=(1, 2), bots=(), board=None):
        ids = list(player_ids)[:2]
        if len(ids) != 2:
            raise ValueError("Treasure Dash is for exactly two players")
        ...
        self.chest = pygame.Vector2(self.width * .5, self.height * .53)
        self.rocks = [(pygame.Vector2(x * self.width, y * self.height), r * self.width) for x, y, r in mirrored(self.board["rocks"])]
        layers = []
        for shapes in (self.board.get("ponds", []), self.board.get("islands", [])):
    ...
    def bot_plan(self, runner):
        ...
        for center, radius in self.rocks:
            free &= np.hypot(xs * cell + cell / 2 - center.x, ys * cell + cell / 2 - center.y) > radius + 22 * self.scale
        free[:round(70 * self.scale / cell)] = False
        free[-2:], free[:, :2], free[:, -2:] = False, False, False
        cost = np.where(self.water[::cell, ::cell][:rows, :columns], 2.4, 1.0)
    ...
    def scribble(self, runner, start, end):
        cv2.line(runner.ink.view(np.uint8), (round(start.x), round(start.y)), (round(end.x), round(end.y)), 1, max(3, round(9 * self.scale)))
    ...
    def start_running(self):
        ...
        if len(ready) < len(self.runners):
            ...
            return
        self.phase, self.timer = "running", self.setting("run_limit", 75)
        self.sounds.append("go")
    ...
    def update_running(self, dt, inputs):
        ...
        for runner in self.runners.values():
            ...
            if special and not runner.special_held and runner.cooldown == 0:
                runner.boost, runner.cooldown = self.setting("boost_seconds", 1.5), self.setting("boost_cooldown", 6)
            ...
            if target <= runner.travelled:
                continue
            pace = self.setting("swim_speed" if swimming else "walk_speed", 12 if swimming else 24) * self.scale
            ...
            if self.rock_at(place, 8 * self.scale) is not None:
                runner.state = "stuck"
            ...
            if runner.travelled >= runner.marks[-1] - 1:
                runner.state = "home"
                self.finish(runner.player_id, "TREASURE!")
                return
        if all(runner.state == "stuck" for runner in self.runners.values()):
            self.finish(None, "BOTH STUCK ON ROCKS")
        elif self.timer <= 0:
    ...
    def bot_trace(self, runner, dt):
        runner.lapse -= dt
        if runner.lapse < -self.rng.uniform(2.2, 4.0):
            runner.lapse = self.rng.uniform(.25, .7)  # Even a good tracer wanders off the line now and then.
        if runner.lapse > 0:
            return None, False
    ...
    def draw(self, surface):
        ...
        for runner in self.runners.values():
            ...
            pygame.draw.circle(frame, sprites.INK, runner.start, 34 * unit)
            pygame.draw.circle(frame, sprites.tint(color, .35), runner.start, 29 * unit)
            if not self.camera_ink or runner.player_id in self.bots:
                for wide, shade in ((13, sprites.INK), (8, color)):
        ...
        chest = self.sprite(("chest", self.phase == "match_over" and self.winner is not None),
                            lambda: sprites.chest(62 * unit, open_lid=self.phase == "match_over" and self.winner is not None))
    ...
    def draw_beaver(self, frame, runner, color):
        ...
        if runner.state == "swimming":
            ring = self.sprite(("ring", color), lambda: sprites.swimmer(16 * unit, color))
        ...
        head = self.sprite(("tim", runner.state == "swimming"), lambda: sprites.tim(22 * unit, paws=runner.state != "swimming"))
        ...
        plate.center = center + pygame.Vector2(0, -46 * unit)
        ...
        if runner.state == "stuck":
            ...
            frame.blit(stuck, stuck.get_rect(center=center + pygame.Vector2(0, 40 * unit)))
    ...
    def draw_hud(self, frame, colors):
        ...
        if self.phase == "running":
            for index, runner in enumerate(self.runners.values()):
                ...
                meter.midleft = (runner.start.x - 60 * unit, runner.start.y + 56 * unit)
                ready = 1 - runner.cooldown / self.setting("boost_cooldown", 6)
        ...
        if self.phase == "match_over":
            ...
            frame.blit(veil, (0, 0))
            plate = pygame.Rect(0, 0, self.width * .62, self.height * .42)
            ...
            title = sprites.label(f"{self.name(self.winner)} WINS!" if self.winner is not None else "NOBODY WINS!", max(24, round(120 * unit)), color, tilt=-4)
            ...
            line = sprites.lettering(self.verdict, max(14, round(44 * unit)), sprites.BLUE)
